...
const Operators = () => {
  ...
  const handleInactivate = async () => {
    try {
      await api.put(`/client_user/inactivate/${operators.selected}`);
      setSnack({ 
  ...
  const handleActivate = async () => {
    try {
      await api.put(`/Client_user/activate/${operators.selected}`);
      setSnack({ 
      ...
      renderCell: (operator) => {        
        return (<>
          { operator.row.admin ? '' : 
            <>
            <IconButton onClick={() => {
              return history.push(`/cli/edit_operator/${operator.row.id}`)}
            }>
              <CreateOutlinedIcon />
            </IconButton>         
  ...
  useEffect(() => {
    ...
    async function getOperators() {
      try {
        const operators = await api.get('/Client_user');    
  ...
  return (
    <ClientDrawer title="Operadores">
      <div className="master-dashboard">
        <BackgroundCard>        
          <BackgroundCardHeader
            title="Operadores"
          >
            <Link className="buttonLink" to="/cli/create_operator">
