...
const CourseList = () => {
  ...
  {/*const handleInactivate = async () => {
    try {
      await api.put(`supplier/inactivate/${classes.selected}`);
      setSnack({ 
  ...
  useEffect(() => {
    ...
    async function getClasses() {
      try {
        ...
        dispatch({ type: 'set', classes: classes.data, });
        setReload(false);
        setLoading(false);
      } catch(err) {        
        setSnack({ 
      ...
      <div className="master-dashboard">
        <BackgroundCard>        
          <BackgroundCardHeader
            title="Turmas"
          >
            <Link className="buttonLink" to="/adm/nova_turma">
